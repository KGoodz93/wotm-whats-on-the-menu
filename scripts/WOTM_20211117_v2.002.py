...
def window_generateshopping():
    ...
    def generateshopping():
        ...
        os.startfile(r"C:\Users\Kelv\Dropbox\Programming\Python\Projects\Completed\WOTM - What's On The Menu!\logs")
    ...
    global count
    sql_shoppinglist = cursor.execute("select *, rowid from _temp;")
...
def window_main():
    ...
    def btncmd_mainmenu_generateshopping():
        ...
        for i in auto1:
            cursor.execute('''INSERT INTO _sorted SELECT * FROM {}'''.format(i[0]), )
            connection.commit()
            f.write(str((i[0])) + "\n")
            f.close()
        ...
        cursor.execute("insert into _temp select sum(quantity) as quantity, ingredients, round(sum(price)) as price from _sorted group by quantity, ingredients, price ORDER BY rowid ASC;")
